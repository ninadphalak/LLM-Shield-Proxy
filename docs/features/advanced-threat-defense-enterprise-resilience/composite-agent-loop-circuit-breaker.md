...
1. **Stateful Trajectory Tracking:** The proxy utilizes the [Stateless Redis TTL Vault](./stateless-redis-ttl-vault.md) to track a hash of the `tool_calls` array for a specific `session_id`.
...
![Agent Circuit Breaker Architecture](../images/composite-agent-loop-circuit-breaker.svg)
...
| :--- | :--- | :--- |
| `ENABLE_AGENT_BREAKER` | Toggles the loop detection engine. | [View in DEPLOYMENT.md](../../DEPLOYMENT.md) |
| `MAX_AGENT_RECURSION` | Maximum consecutive identical tool payloads allowed (default 3). | [View in DEPLOYMENT.md](../../DEPLOYMENT.md) |
