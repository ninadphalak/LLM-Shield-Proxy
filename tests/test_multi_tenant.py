...
def test_multi_provider_routing_openai(monkeypatch, httpx_mock):
    ...
    monkeypatch.setattr(
        "llm_shield_proxy.core.config.settings._valid_virtual_keys_set", frozenset(["sk-proxy-tenant-a"])
    )
...
def test_multi_provider_routing_gemini(monkeypatch, httpx_mock):
    ...
    monkeypatch.setattr(
        "llm_shield_proxy.core.config.settings._valid_virtual_keys_set", frozenset(["sk-proxy-tenant-b"])
    )
    monkeypatch.setattr("llm_shield_proxy.core.config.settings.GEMINI_API_KEY", "AIza-central-gemini-key")
    monkeypatch.setattr(
        "llm_shield_proxy.core.config.settings.UPSTREAM_BASE_URL",
