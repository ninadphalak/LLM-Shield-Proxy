...
claim to redact PII, so a privacy benchmark must not classify it as a redaction failure.
The same distinction applies to Presidio's replace/hash/mask operators, which leak
nothing and cannot rehydrate.
...
def test_product_with_no_redaction_feature_is_not_a_failure(capture_port):
    ...
    The gateway forwards the fixture verbatim -- every leak
...
def test_redaction_claimed_and_enabled_and_leaking_is_a_real_fail(capture_port):
    """A leak verdict requires the claim, configuration, and measurement to align."""
...
def test_a_claim_without_a_citation_is_refused():
    """A product claim requires a supporting citation before outcome classification."""
...
def test_one_way_anonymizer_is_not_reported_as_a_leak(capture_port):
    ...
    rehydration, so a correctly configured Portkey run lands exactly here. It must be
    classified as a no-leak profile mismatch rather than a leak.
    """
