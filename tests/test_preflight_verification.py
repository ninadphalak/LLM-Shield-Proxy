...
def test_tenant_isolation_and_ssrf(httpx_mock):
    with TestClient(app) as client:
        ...
        res_ssrf = client.post(
            "/v1/chat/completions",
            headers={
                "authorization": "Bearer sk-proxy-team-a",
                "x-upstream-base-url": "http://169.254.169.254"
            },
        ...
        res_b = client.post(
            "/v1/chat/completions",
            headers={
                "authorization": "Bearer sk-proxy-team-b",
                "x-session-id": "sess_prod"
            },
    ...
    with TestClient(app) as client:
        ...
        res = client.post(
            ...
            json={
                "model": "gpt-4",
                "stream": True,
                "messages": [{"role": "user", "content": "What was the contact?"}]
            }
        )
    ...
    with TestClient(app) as client:
        with client.stream("POST", "/v1/chat/completions", 
            ...
            for _ in response.iter_bytes():
                ...
                assert mem_during - mem_before < 26214400, f"Active memory exceeded 25MB during stream: {mem_during - mem_before} bytes"
