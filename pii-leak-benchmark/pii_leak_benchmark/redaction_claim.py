...
This module keeps the published classification within the target's documented scope.
For example, Cloudflare AI Gateway offers caching, rate limiting, observability, and
routing but does not claim to redact PII. A privacy test can still record measurements
for it, but the result must not be classified as a redaction failure. The same distinction
applies to Presidio's ``replace``/``hash``/``mask`` operators, which leak nothing and
cannot rehydrate.
...
Miss any one and the run still produced valid measurements -- they are published
...
def normalize_claim(claim: Optional[dict[str, Any]]) -> dict[str, Any]:
    ...
    Raises ValueError rather than guessing. A wrong default would misclassify the
    target's documented capability, so there is no permissive path.
    """
    if claim is None:
    ...
    if claimed != CLAIM_UNKNOWN and not citation:
        # A claim without a citation cannot be checked independently.
        raise ValueError(
