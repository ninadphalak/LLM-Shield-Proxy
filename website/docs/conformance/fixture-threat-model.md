...
- **It favored non-validating detection.** All three values were *invalid specimens*, so a
  detector that validates its input correctly ignored them while a shape-matching regex engine
  caught them. **Fixed on 2026-09-01**, and the measurement that exposed it is preserved below.
- **Format-specific handling can pass this profile.** A target that matches the three
...
The reference implementation passed both because `engines/pii_engine.py` Tier 1 is bare
...
the correctly redacting reference implementation**. A one-in-three false leak rate is worse than the
