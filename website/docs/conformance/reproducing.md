...
A hosted gateway cannot open a connection to your loopback, which for a long time made
every hosted target unmeasurable. The fix is not a service this project operates — a
neutral referee cannot also host the traffic it measures, and it would be a standing
hosting burden and a conflict of interest. Instead **you** deploy the capture: on your own
...
- `unattributed_requests` — requests that did not present the capture token. Expected to
  be non-zero on a public address. **Does not fail the check**: gating on it would fail an
  honest run for something a stranger did.
...
- `unattributed_leaked_entity_types` — protected fixture values found in unattributed
  traffic. This **does** fail the boundary check, because the fixture is synthetic and no
  honest third party sends it. It is reported in its own field rather than folded into
  ...
  manufacture a leak finding against an honest gateway.
...
forwards only `/v1/*`, or that rewrites the prefix, does not turn an honest setup into an
...
the host, so aborting there would fire on an honest setup. The field is published instead,
