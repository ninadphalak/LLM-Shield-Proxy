...
> **First independent pass, unclaimed.** No gateway we did not write has passed yet. The
> first one to do it gets named here with the date and the exact version, and that entry
> stays whatever happens afterwards.
