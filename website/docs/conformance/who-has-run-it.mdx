...
is on the [results page](./results).
...
**Nobody else has passed yet.** The first gateway we did not write that answers all three
gets a mark on its row in the table below, and keeps it. It is worked out from the results
rather than handed out, so it cannot be claimed, only earned.
