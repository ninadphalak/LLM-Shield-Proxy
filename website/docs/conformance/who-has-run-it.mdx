...
**1. Run it in your CI.** Copy [the CI setup](./ci) and change three lines: where your gateway
...
cleanly. The run writes `pii-leak-badge.json` beside its other reports; publish that file
from your own repository. Nothing is hosted here and there is nothing to register.

**Anywhere else**, link this page. It carries a preview card, and the row is yours to point at:
...
- [Share on X](https://twitter.com/intent/tweet?url=https%3A%2F%2Fllmshieldproxy.com%2Fdocs%2Fconformance%2Fwho-has-run-it&text=We%20ran%20the%20PII%20leak%20check%20on%20our%20LLM%20gateway.%20Here%20is%20what%20it%20found%3A)
