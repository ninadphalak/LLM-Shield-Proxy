...
Nothing running yet? [Try it from scratch](./try-it-from-scratch) starts with an empty machine
...
exact run. If the run happened in CI, that block is already in the job summary, no download
needed.

**3. Post it.** One command, from wherever the report is:

```bash
pii-leak-benchmark submit
```

It finds the report, builds the citation and opens the submission, filled in. With the GitHub
CLI installed it posts the issue directly; without it, your browser opens on the form. The report
itself is never uploaded, only the citation block: a report can hold your gateway's base URL and
your capture hostname, and this goes to a public tracker.

`submit` arrived in `pii-leak-benchmark` 0.3.2. On an older one, use the form below.

If you would rather do it by hand, [open the form](https://github.com/ninadphalak/LLM-Shield-Proxy/issues/new?template=conformance-result.yml)
and paste the block in. A run from CI also puts a one-click link straight in the job summary.

### What happens after you post

Nobody approves it. A check reads the submission as soon as it arrives, opens the CI run you
linked, **downloads that run's own build artifact, and reads the numbers out of it**. Then it
builds this site to prove the row does not break it, commits, and closes your issue with the
link. A few minutes, start to finish.

The numbers are never retyped from your issue. That is the point of requiring a public run
rather than a pasted report: there is nothing to mistype, nothing for us to shade in your
favour or against it, and nothing you could send that would not also have to exist as a public
Actions run in a named repository on a named branch. That branch is also what fills in the "Who
ran it" column.

What this establishes is where the run happened and what it recorded. Not that your gateway was
the version you named, which nothing outside your repository can establish. The three things a
report does not record, the gateway's name, its licence and how it reads a stream, are yours to
state and are marked as self-reported like everything else on this page.

If the artifact cannot be read, and an expired one often cannot, the check says so on the issue
and publishes nothing. Rerun and resubmit; a second submission replaces your row rather than
adding a second one.
