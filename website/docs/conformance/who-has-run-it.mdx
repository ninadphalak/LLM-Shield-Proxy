...
> **First to pass, unclaimed.** Nobody has answered all three questions yet. The first
> gateway that does gets named here with the date and the exact version it passed on, and
> that entry stays whatever happens afterwards. It is a milestone rather than a placing:
> there is no second and no ranking, only the date something first worked.
