...
## One gateway passes, and it is ours

We would rather that read as awkward than as a boast. We wrote the check, we ran it against our
own product first, and we published the result when it failed: the 1.6.0 rows below show the six
cases we leaked, and they are staying there. Version 1.6.6 fixed them. We were quick to fix them
because we knew exactly where to look, which is an advantage nobody else measured here had.

So treat the passing row as a claim to check rather than a result to trust. The check is the
useful part, not our score on it. **If you find a case that trips us, send it.** It goes into the
corpus, every future run gets stricter, and your name goes on this page.
...
- **It passed.** Say so loudly. You are the second.
