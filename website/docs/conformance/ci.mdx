...
- **Where the reports are**, spelled out, because "it is in the artifact" is not directions:
  ...
  reports.
- **A one-click link** to the [results wall](./who-has-run-it), prefilled with the version, the
...
The Action renders it from the operator result, so it names the types rather than a rate:

| Badge reads | Colour | What it means |
| :--- | :--- | :--- |
| `contained` | green | No test value reached the upstream and the required checks passed |
| `leaked: EMAIL, SSN` | red | Those data types were seen arriving upstream, listed in full |
| `checks failed` | yellow | Nothing leaked, but a required check did not hold |
| `not measured` | grey | Setup, attribution, inspection or comparison was incomplete |
