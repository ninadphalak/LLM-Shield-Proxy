...
The same trip back is worth as much. A gateway that masks on the way out and never puts the
value back is not leaking, but it is handing your caller a placeholder where its own data
should be, and that breaks the app just as effectively. The capture replies with the message
it received, one character at a time, so the value your gateway has to rebuild arrives split
across as many events as it has characters. `response_fidelity` records whether the caller got
its own text back and `fragmentation_safety` records whether it survived being split, which is
why a run can report no leak and still fail.
...
- **The citation block**, in a collapsible section. It names the harness revision, the test data
  and the configuration the numbers came from, so a reader can rerun your exact run. It is right
  there in the page, so nobody has to download a zip and run a second command to get it.
- **Where the reports are**, spelled out, because "it is in the artifact" is not directions:
  ...
  2. Click the run, which is the one named after your workflow.
