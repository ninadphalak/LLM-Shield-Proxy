...
If you want to build and measure the proxy source in its own fork, start with a
[copyable source-build recipe](./source-ci.md) for LLM-Shield-Proxy, Portkey, LiteLLM,
or NeMo Guardrails. The generic Action below assumes you provide the gateway startup
command.
...
A workflow that uploads its own evidence bundle, such as the
[proxy source-build recipes](./source-ci.md), sets `submission-section: 'false'` and writes a
submission link that points at that bundle instead.
