...
is missing. The intake downloads the linked public run’s recognized report artifact, derives the
measurement columns from its JSON reports, builds the site, and publishes the row through a
...
For ordinary operator CI and regression comparisons, use the repository's [root composite
Action](https://github.com/ninadphalak/LLM-Shield-Proxy/blob/main/action.yml). It produces operator
and raw reports, a job summary, and a result badge. It does not create a detached attestation. The
[CI guide](./ci.mdx) documents that interface.

The nested [research HTTP-profile
Action](https://github.com/ninadphalak/LLM-Shield-Proxy/tree/main/.github/actions/pii-leak-benchmark)
is a legacy compatibility surface for research submissions that require detached provenance. The
following example remains supported for that purpose:
