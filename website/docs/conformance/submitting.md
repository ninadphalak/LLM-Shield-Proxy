...
The [results table](./results) exists to be contradicted. This page is the whole submission
path: what a run must carry, how it is checked, and what makes a row count as replicated.
...
A run with an artifact and no configuration is unpublishable, because nobody can reproduce it. A
...
| `marker_words_observed_max` | Distinguishes an over-redacting honest gateway from a target that sent its traffic elsewhere |
...
| `needle_proximity` / `needle_lengths` | The margin on this specific run. An honest run measures SSN 2 of 9 |
| `redaction_claim.claim_citation` resolves and supports the claim | An unsourced assertion about someone else's product is the one thing a referee may not make |
...
heard of, and **especially** for runs that contradict a published row.
