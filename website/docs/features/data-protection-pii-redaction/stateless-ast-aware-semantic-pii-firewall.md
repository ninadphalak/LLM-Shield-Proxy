...
2. **AST-Aware Traversal:** The engine parses the payload and targets sensitive leaf values in `tool_calls[*].function.arguments`. It runs the configured detector tiers on extracted values. *(See [Supported PII Types](supported-pii-types.md).)*
