...
bundles NIST OSCAL assessment-results artifacts, a verified WORM audit-log summary, and a
SHA-256 file-integrity manifest into a single `.zip`, with a generated Markdown summary an
auditor can read directly.

## How It Works
1. **Audit Evidence Verification:** Given `--audit-log`, the CLI re-walks the WORM JSONL
...
```mermaid
flowchart TD
    A[WORM Audit Log] --> B[Verify Hash Chain + Ed25519 Signatures]
...
## Performance Profile
- **Execution Speed:** A pure offline CLI operation — verification is a single linear pass
...
| `--audit-log` | Path to a WORM audit log JSONL file (captured proxy stdout). Omit to skip audit evidence. |
...
A: No. It operates entirely on files you already have — a captured WORM audit log and an
