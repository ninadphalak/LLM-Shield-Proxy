...
it - **never one without the other**. **None of it is an independent benchmark.** Every measured
row comes from the initial project-run measurement set on one Windows workstation, and none has
been independently reproduced.
Every target therefore reads `unreplicated` in the published table, which needs 3 runs from 3
distinct submitters before a result is treated as replicated. See
...
| `http-profile-llm-shield-proxy-working-tree.json` | This project | `pass` | A project-run measurement of the reference implementation. One run, one submitter, unreplicated. Configuration and known limits in `http-profile-llm-shield-proxy-working-tree.md`. |
...
genuinely produced — two real defects in the reference implementation, both now fixed and pinned
...
the reference implementation's detector design; that is recorded in
