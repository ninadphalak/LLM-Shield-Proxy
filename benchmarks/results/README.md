...
it. **None of it is an independent benchmark.** Every measured row was produced by this
...
and it is disclosed rather than hidden. See
...
| `http-profile-llm-shield-proxy-working-tree.json` | This project | `pass` | A maintainer self-test of an uncommitted working tree. Not a release result. Configuration and known limits in `http-profile-llm-shield-proxy-working-tree.md`. |
...
on sample completeness. Three iterations is a smoke test. Nothing in these files should be
read as a speed comparison. The configuration records preserve a maintainer-local latency
diagnostic that found two hot-path defects in **this** proxy, but its runner and raw samples
were not retained. Those exact figures are not independently auditable benchmark evidence.
