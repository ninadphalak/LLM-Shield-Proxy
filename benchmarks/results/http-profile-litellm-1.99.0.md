...
Two project-run rows against a locally installed LiteLLM proxy. Neither has been independently
reproduced. See
...
`leaked: ["SSN"]` and would have been classified as `fail` because the fixture's SSN was a value
Presidio deliberately rejects. The fixture was corrected before the row was published.
...
be re-derived on demand. A later independent re-measurement of one component also found a
materially different magnitude. The unsupported comparison was therefore removed.
